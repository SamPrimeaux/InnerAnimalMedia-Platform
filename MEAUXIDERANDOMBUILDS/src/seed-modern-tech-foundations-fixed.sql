-- Seed: Modern Tech Foundations for Builders (Fixed)
-- Complete course with modules, lessons, and quizzes
-- Run: wrangler d1 execute inneranimalmedia-business --file=src/seed-modern-tech-foundations-fixed.sql --remote
...
- mv old-name.txt new-name.txt - Move/rename a file
- rm filename.txt - Delete a file
- rm -rf folder-name - Delete a folder (careful!)

Viewing Files:
- cat filename.txt - View file contents
- less filename.txt - View with pagination (press q to quit)
...
4. Commit: git commit -m "Fix bug in login"
...
Must-Have Extensions for VS Code/Cursor:
...
Example Request:
Use fetch() to make API calls. GET requests retrieve data, POST requests send data.',
  1,
  20,
  1,
  1,
